...
![The GPU memory hierarchy: huge, slow HBM vs tiny, fast on-chip SRAM. Naive attention shuttles the n×n score matrix through HBM; FlashAttention keeps every intermediate inside SRAM tiles.](../../../../models-and-architectures/attention-and-transformers/efficient-attention/images/fa_hbm_sram.png)
...
![FlashAttention's tiling structure: the query block holds still while K/V blocks stream through SRAM one tile at a time; the running output accumulates per tile and the full score matrix never exists.](../../../../models-and-architectures/attention-and-transformers/efficient-attention/images/fa_tiling_structure.png)
...
![Online softmax in motion: each incoming tile updates the running max m, denominator ℓ, and output o — old accumulators rescaled by e^(Δm) — and after the last tile o/ℓ equals the exact softmax-weighted output.](../../../../models-and-architectures/attention-and-transformers/efficient-attention/images/fa_online_softmax.png)
...
![What IO-awareness buys: attention memory traffic and peak memory vs sequence length, naive kernel vs FlashAttention — the O(n²) HBM traffic term is gone.](../../../../models-and-architectures/attention-and-transformers/efficient-attention/images/fa_memory_savings.png)

> **Gotcha:** FlashAttention is a *kernel*, not an architecture. It composes with every variant in [chapter 1](/ai-ml/ai-ml-learning-resources/inference-and-serving/kv-cache/kv-cache-variants) — GQA, MLA, quantized caches — because it only changes how the bytes are scheduled, never which bytes exist.
...
- The **cache levers** ([chapters 1](/ai-ml/ai-ml-learning-resources/inference-and-serving/kv-cache/kv-cache-variants)–[2](/ai-ml/ai-ml-learning-resources/inference-and-serving/kv-cache/kv-cache-optimization-stack)) set *how many bytes* must move per step: KV heads × bits × tokens × allocation efficiency.
...
Next: [Chapter 4 — the KV cache in production](/ai-ml/ai-ml-learning-resources/inference-and-serving/kv-cache/kv-cache-in-production): schedulers, metrics, and the incidents.

---

## References

Shared with the topic's companion file — see [KV Cache — references and further reading](/ai-ml/ai-ml-learning-resources/inference-and-serving/kv-cache/kv-cache#references-further-reading) (FlashAttention, FlashDecoding, and serving-kernel entries).
