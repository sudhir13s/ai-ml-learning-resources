...
1. **Understand *why* environments exist** — watch [Python Tutorial: VENV](https://www.youtube.com/watch?v=Kg1Yvry_Ydk) — **Corey Schafer**. *Isolation and dependency clashes, made concrete.*
...
3. **Pin what you install** — produce a lockfile, not just a `requirements.txt`, and commit it. *An environment you cannot recreate exactly is a result you cannot reproduce.*
4. **Meet the modern tool** — watch [UV: a faster all-in-one package manager](https://www.youtube.com/watch?v=AMdG7IjgSPM) and read the [uv project guide](https://docs.astral.sh/uv/guides/projects/). *`uv` now covers Python versions, virtual environments, resolution and locking in one tool, and has become the default choice for new projects.*
...
## Videos
- [Python Tutorial: VENV (Mac and Linux)](https://www.youtube.com/watch?v=Kg1Yvry_Ydk) — **Corey Schafer** — the clean, canonical `venv` walkthrough: create, activate, freeze, remove.
- [UV — a faster, all-in-one package manager to replace pip and venv](https://www.youtube.com/watch?v=AMdG7IjgSPM) — **Corey Schafer** — the modern uv workflow, including lockfiles and per-project Python versions.
- [Python Tutorials (full playlist)](https://www.youtube.com/playlist?list=PL-osiE80TeTt2d9bfVyTiXJA-UTHn6WwU) — **Corey Schafer** — the surrounding series: imports, modules, packaging, and the path issues that cause most "it works on my machine" reports.
...
- Go deeper — reproducibility, containers, MLOps: [12. Deployment & MLOps](/ai-ml/ai-ml-learning-resources/operations-and-lifecycle/readme)
