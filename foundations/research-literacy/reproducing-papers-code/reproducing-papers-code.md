...
1. **Find existing code first** — check the paper's own repository, then [Hugging Face Papers](https://huggingface.co/papers) (it links the models and Spaces built from a paper) before writing anything. *Running the authors' code is the fastest way to confirm the result and read the real implementation.*
...
4. **Use a reproducibility checklist** — apply the [ML Reproducibility Checklist](https://www.cs.mcgill.ca/~jpineau/ReproducibilityChecklist.pdf), now the [NeurIPS paper checklist](https://neurips.cc/public/guides/PaperChecklist), plus the [releasing-research-code](https://github.com/paperswithcode/releasing-research-code) practices. *Checklists surface the undisclosed details (seeds, splits, hyperparameters) that make or break a reproduction.*
...
- [Let's reproduce GPT-2 (124M)](https://www.youtube.com/watch?v=l8pRSuU81PU) — **Andrej Karpathy** — a full paper reproduced from scratch, including the details the paper leaves implicit: initialization, learning-rate schedule, data pipeline, and the final loss it has to match.
...
- [Hugging Face Papers](https://huggingface.co/papers) — **Hugging Face** — the live paper-to-code index since Papers with Code shut down in 2025: each paper links the models, datasets and Spaces built from it.
- [Releasing Research Code — best practices](https://github.com/paperswithcode/releasing-research-code) — **Papers with Code (repository still public)** — the README/repository checklist that makes, and judges, reproducible releases.
- [ML Reproducibility Challenge — reports](https://reproml.org/blog/) — **MLRC** — real attempts to reproduce accepted papers; read them to see what breaks in practice.
- [NeurIPS paper checklist](https://neurips.cc/public/guides/PaperChecklist) — **NeurIPS** — the disclosure questions every author must now answer; use it as your inspection list when a repository omits something.
