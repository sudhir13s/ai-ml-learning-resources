...
- **LoRA — low-rank adaptation of LLMs.** Fine-tuning a giant weight matrix $W$ by learning a **low-rank update** $\Delta W = BA$ (rank $r \ll d$) is the Eckart–Young insight applied to *weight deltas*: the useful adaptation lives in a low-rank subspace, so you train $r(d+d)$ parameters instead of $d^2$. This is why a 70B model can be fine-tuned on a single GPU. ([LoRA & PEFT](/ai-ml/ai-ml-learning-resources/model-adaptation/lora-and-parameter-efficient-fine-tuning/lora-and-parameter-efficient-fine-tuning).)
