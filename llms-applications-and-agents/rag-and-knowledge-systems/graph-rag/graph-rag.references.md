...
> Companion link library for **[GraphRAG](/ai-ml/ai-ml-learning-resources/llms-applications-and-agents/rag-and-knowledge-systems/graph-rag/graph-rag)** (the concept page). External sources *and* internal cross-links, kept separate so it can be reused as a standalone list. Grouped by type, alphabetical within each group. Every entry is free/open (no paywall) and chosen for depth on *this* topic — knowledge-graph construction, multi-hop traversal (local search), community detection (modularity / Louvain / Leiden), and global query-focused summarization.
