...
- reuse **RRF** ([chapter 5](../hybrid-search/hybrid-search.md)) to fuse the reformulations' lists;
...
3. **Fuse (Multi-Query only).** Combine the N lists into one. The robust default is **Reciprocal Rank Fusion** — the exact rank-based fusion [chapter 5](../hybrid-search/hybrid-search.md) built — which needs no score normalization and rewards a passage ranked high by *any* reformulation. Dedupe (the same passage retrieved by several reformulations must count once), then keep the final top-k.
4. **Downstream is unchanged.** The final passages go to a [reranker](../reranking/reranking.md) or straight into the [augmented prompt](../rag-foundations/rag-foundations.md). Transformation slots in *ahead* of everything else in the stack.

> **Note:** HyDE and Multi-Query **compose** — and with hybrid search too. **RAG-Fusion** is literally "Multi-Query + RRF." You can run HyDE to write a hypothetical, then Multi-Query to diversify it, then hybrid ([chapter 5](../hybrid-search/hybrid-search.md)) on each, then RRF the whole pile. Each stage is another LLM call and another fan-out, so you trade latency and cost for recall — the engineering judgment the last two sections sharpen.
...
(the dot product because the vectors are unit-norm — [chapter 3's](../embedding-models/embedding-models.md) geometry), and returns the top-k. **HyDE changes exactly one thing:** it first has an LLM $G$ generate a hypothetical answer $h = G(q)$, then scores with $E(h)$ in place of $E(q)$:
...
> *Where this comes from: **Cormack, Clarke & Büttcher, "Reciprocal Rank Fusion outperforms Condorcet and individual Rank Learning Methods" (SIGIR 2009, [DOI:10.1145/1571941.1572114](https://doi.org/10.1145/1571941.1572114))** — the $k=60$ default and the "high in any list wins" behaviour are theirs, worked through in full in [chapter 5](../hybrid-search/hybrid-search.md). Source in the [references](query-transformation.references.md).*

---

## Code: measure the transform on a real encoder

Here's the honest experiment. The retrieval, encoder, and metrics are **real** — the `all-MiniLM-L6-v2` bi-encoder and the RRF/MRR/recall machinery imported straight from [chapter 5](../hybrid-search/hybrid-search.md) (which imports [chapter 1's](../rag-foundations/rag-foundations.md) corpus). The **generation is represented by fixed, labelled exemplars** standing in for an LLM. It runs on CPU in a couple of seconds.
...
The raw query already scores 0.932 and ranks the gold **#1** — there is nothing to fix, and a hypothetical could only move it. *Rule:* **skip transformation for exact-keyword, code, and already-precise queries;** reach for it on **short, vague, or vocabulary-mismatched** ones. Lexical/hybrid search ([chapter 5](../hybrid-search/hybrid-search.md)) already handles exact tokens — transformation is for the semantic-gap cases.
...
| Exact codes, keywords, IDs | **skip** → hybrid/BM25 ([ch5](../hybrid-search/hybrid-search.md)) | lexical nails exact tokens; a hypothetical can only blur them |
...
- **RRF default.** $k=60$, from Cormack et al. (2009) — the same constant [chapter 5](../hybrid-search/hybrid-search.md) and Elasticsearch's RRF use.
...
**→ [Query Transformation (HyDE & Multi-Query) — references and further reading](query-transformation.references.md)**
