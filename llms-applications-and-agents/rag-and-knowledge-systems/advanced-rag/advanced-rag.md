...
- recognize **RAG-Fusion** as multi-query + RRF and reuse [chapter 7's](../query-transformation/query-transformation.md) machinery;
...
We reuse [chapter 2's](../chunking/chunking.md) multi-section Helios-7 spec. The natural hierarchy: each **section** (`# Orbit`, `# Instruments`, …) is a **parent**; each **sentence** in it is a **child**. Index the *children* (small, sharp embeddings) and ask *"What keeps the local solar time of each pass roughly constant?"*. The real encoder retrieves this child at #1:
...
> **Source / derivation:** [Cormack, Clarke & Büttcher, "Reciprocal Rank Fusion outperforms Condorcet…" (SIGIR 2009)](https://doi.org/10.1145/1571941.1572114) — **RAG-Fusion** = multi-query expansion + this RRF ($k=60$), the fusion [chapter 5](../hybrid-search/hybrid-search.md) and [chapter 7](../query-transformation/query-transformation.md) build and reuse. The `retrieve_multiquery` function here is imported directly from chapter 7. Source in the [references](advanced-rag.references.md).
...
| **RAG-Fusion** | queries are vague/underspecified and a single probe misses facets | multi-query + RRF ([ch7](../query-transformation/query-transformation.md)); RAG-Fusion implementations |
...
- *What is RAG-Fusion?* Multi-query expansion + RRF ([ch7](../query-transformation/query-transformation.md)) — recovers facts a single vague query misses.
