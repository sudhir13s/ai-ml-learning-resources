...
- [Not what you've signed up for: Compromising Real-World LLM-Integrated Applications with Indirect Prompt Injection](https://arxiv.org/abs/2302.12173) — **Greshake et al. (2023)** — the attack that defines agent security: instructions hidden in *retrieved* content (a web page, a document, a tool result) are read by the model as if the user had typed them. Any agent that reads untrusted data has this hole by construction.
...
- [Prompt injection — the ongoing series](https://simonwillison.net/series/prompt-injection/) — **Simon Willison** — the running record of real attacks against shipped agents, and the case that no prompt-level defense has held; the reason serious guardrails are architectural (capability limits, isolated contexts) rather than instructional.
- [OWASP Top 10 for LLM Applications](https://genai.owasp.org/llm-top-10/) — **OWASP GenAI Security Project** — the industry checklist an interviewer may expect you to name: prompt injection, insecure output handling, excessive agency, and supply-chain risk, each with mitigations.
- [Writing tools for agents](https://www.anthropic.com/engineering/writing-tools-for-agents) — **Anthropic (2025)** — safety framed as interface design: narrow tools with explicit scopes and readable errors constrain an agent far more reliably than instructions telling it to behave.

For the attack surface and its defenses in depth, see [Prompt Injection and Agent Guardrails](/ai-ml/ai-ml-learning-resources/llms-applications-and-agents/agentic-ai/prompt-injection-and-agent-guardrails/prompt-injection-and-agent-guardrails).
...
- Related (canonical home): [Hallucination & Alignment basics](/ai-ml/ai-ml-learning-resources/evaluation/alignment-and-safety-evaluation/alignment-and-safety-evaluation) · [RLHF & DPO](/ai-ml/ai-ml-learning-resources/model-adaptation/preference-and-alignment-training/preference-and-alignment-training)
- Prompt injection specifically — the lethal trifecta, indirect injection, OWASP LLM01 and design-level defences — is owned by [Prompt Injection and Agent Guardrails](/ai-ml/ai-ml-learning-resources/llms-applications-and-agents/agentic-ai/prompt-injection-and-agent-guardrails/prompt-injection-and-agent-guardrails); this page keeps the broader guardrail and human-in-the-loop scope.
