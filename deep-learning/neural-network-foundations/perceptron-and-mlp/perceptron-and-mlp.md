...
> **Note:** the **transformer's feed-forward block is exactly this MLP** — `Linear(d, 4d) → GELU → Linear(4d, d)`, applied independently at every position. It typically holds **~two-thirds of a transformer's parameters**. So when you study attention in the [transformer](/ai-ml/ai-ml-learning-resources/models-and-architectures/attention-and-transformers/transformer-architecture/transformer-architecture), remember the FFN sitting beside it *is the humble MLP from this page*, just very wide. The perceptron never left; it became the workhorse inside the most important architecture in AI.
