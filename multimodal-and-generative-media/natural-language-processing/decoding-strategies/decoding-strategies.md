...
The idea exploits the fact that autoregressive **decode is memory-bandwidth-bound** (see [09 LLMs · KV Cache](/ai-ml/ai-ml-learning-resources/inference-and-serving/kv-cache/kv-cache)): each step the big model mostly waits on memory, doing little compute, so verifying *several* tokens in one forward pass costs barely more than generating one. So:
...
**Decoding choice changes your evaluation numbers.** The decoder is part of the system under test, so it must match the metric ([18 NLP Evaluation Metrics](/ai-ml/ai-ml-learning-resources/multimodal-and-generative-media/natural-language-processing/nlp-evaluation-metrics/nlp-evaluation-metrics)):
...
**→ [Decoding Strategies — references and further reading](/ai-ml/ai-ml-learning-resources/multimodal-and-generative-media/natural-language-processing/decoding-strategies/decoding-strategies#references-further-reading)**
