...
> **Source / derivation:** the log-normalized (sublinear) term-frequency transform $1 + \log f$ and the binary/augmented variants are the SMART term-weighting family catalogued in [Salton & Buckley, *Term-weighting approaches in automatic text retrieval* (1988)](https://doi.org/10.1016/0306-4573%2888%2990021-0) and tabulated in [Manning, Raghavan & Schütze, *Introduction to Information Retrieval*, Ch. 6 §6.4](https://nlp.stanford.edu/IR-book/html/htmledition/sublinear-tf-scaling-1.html). Both are in the references.
...
This is the link to [n-gram language models](/ai-ml/ai-ml-learning-resources/multimodal-and-generative-media/natural-language-processing/n-gram-language-models-and-smoothing/n-gram-language-models-and-smoothing): the same contiguous-window idea, used there for *prediction* and here for *features*. In practice `TfidfVectorizer(ngram_range=(1,2))` mixes unigrams and bigrams, and bigrams reliably lift text-classification accuracy a few points — capturing negation, fixed phrases (`new york`, `machine learning`), and short idioms.
...
1. **Preprocess** — lowercase, tokenize, strip punctuation. Decide on stopword removal (usually yes for retrieval, sometimes no for classification where function words carry signal) and stemming/lemmatization (collapses `running`/`ran`/`runs`). See [Text Preprocessing](/ai-ml/ai-ml-learning-resources/multimodal-and-generative-media/natural-language-processing/text-preprocessing-and-normalization/text-preprocessing-and-normalization).
...
- **No semantics (synonymy).** Distinct words are orthogonal axes; `car` and `automobile` have similarity 0. → [Word embeddings](/ai-ml/ai-ml-learning-resources/multimodal-and-generative-media/natural-language-processing/word-embeddings-word2vec-glove-fasttext/word-embeddings-word2vec-glove-fasttext) place related words *near* each other so similar meaning ⇒ similar vector.
...
- **No polysemy resolution.** One vector per word type, all senses merged. → [Contextual embeddings](/ai-ml/ai-ml-learning-resources/multimodal-and-generative-media/natural-language-processing/contextual-embeddings-elmo-bert/contextual-embeddings-elmo-bert) (ELMo, BERT) give a *different* vector per occurrence based on context.
- **Out-of-vocabulary words.** Unseen terms have no column and vanish. → Subword [tokenization](/ai-ml/ai-ml-learning-resources/multimodal-and-generative-media/natural-language-processing/tokenization-and-subword-algorithms/tokenization-and-subword-algorithms) (BPE, WordPiece) and FastText's character n-grams handle novel words.
...
**→ [Bag-of-Words & TF-IDF — references and further reading](/ai-ml/ai-ml-learning-resources/multimodal-and-generative-media/natural-language-processing/bag-of-words-and-tf-idf/bag-of-words-and-tf-idf#references-further-reading)**
