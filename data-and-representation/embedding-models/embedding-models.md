...
tier: core
est_minutes: 32
core_idea: "An embedding model is trained rather than configured: contrastive learning decides which texts end up close together, and every search built on top can only rank what that learned geometry already put nearby."
...
When a query and its answer use *different words*, a retriever built on word overlap misses — "liftoff date" shares nothing with "was launched on." That is **not** a chunking problem or an index problem. It is decided by one model, the **embedding model**, and this page is about how that model is built.
...
A **lexical / sparse** embedder represents text as a bag of words: one vector slot per vocabulary word, nonzero only where that word literally appears (TF-IDF is the classic example; a hashing embedder is a close cousin). Two texts are similar *only if they share words*. That's fine for "Helios-7 launch date" → "Helios-7 was launched", but it falls apart the moment the query is a **paraphrase**:
...
Picture a small help-centre corpus embedded and squashed down to two dimensions for viewing:

<img src="images/emb_2d_scatter.png" alt="A 2D scatter plot of embedded sentences. Three coloured clusters: blue refunds documents in the upper left, green shipping documents in the upper right, and purple accounts documents in the lower middle. A red star marks the query 'how long do refunds take?' inside the blue refunds cluster, with a dashed red circle around it labelled 'top-k = nearest dots'." width="720">

*Illustrative: synthetic 2D points standing in for projected sentence embeddings (no embedder produced them). Generated by `code/make_figures_03.py`.*

- **Topics become neighbourhoods.** Refund, shipping and account sentences form three separate clusters.
- **The query lands inside the cluster that answers it.** Retrieval is then just "grab the dots inside the circle".
- **Real embeddings have hundreds of dimensions.** A 2D view is a squashed projection, so treat distances in it as a rough picture.

### Directions carry relations, not just topics

In word-level embeddings, a relation shows up as an arrow — and the same arrow repeats across pairs:

![A 2D illustration of word embeddings as points in space. Country names (France, Germany, ...) sit at one set of points and their capitals (Paris, Berlin, ...) at another, with parallel arrows of the same direction and length connecting each country to its capital, showing that the country-to-capital relationship is a consistent vector offset shared across pairs.](https://upload.wikimedia.org/wikipedia/commons/f/fe/Word_embedding_illustration.svg)

*Figure: meaning is direction — the country→capital relationship is the same vector offset for every pair (France→Paris, Germany→Berlin), so embeddings answer analogies by arithmetic — Wikimedia Commons, Fschwarzentruber ([link](https://commons.wikimedia.org/wiki/File:Word_embedding_illustration.svg)), CC BY-SA 4.0.*

- **Offsets encode relations.** `Paris − France ≈ Berlin − Germany` is the analogy arithmetic popularised by [word2vec (Mikolov et al., 2013, arXiv:1301.3781)](https://arxiv.org/abs/1301.3781).
- **Treat it as intuition for sentence embedders, not a guarantee.** Retrieval models are trained for nearness, not analogy, so clean parallel arrows are not promised there.

### A real embedding space, not a toy

Here are real word embeddings, learned from a text corpus and projected to 2D with t-SNE:

![A 2D t-SNE projection of word embeddings learned from 19th-century literature. Words appear as a dense cloud of points that separate into several visibly distinct clusters labeled A through D, with gender-related, occupational, family, and pronoun terms grouping into their own neighbourhoods, illustrating that semantically related words land near each other in embedding space.](https://upload.wikimedia.org/wikipedia/commons/9/94/T-SNE_visualisation_of_word_embeddings_generated_using_19th_century_literature.png)

*Figure: a real embedding space — word embeddings projected to 2D with t-SNE form distinct semantic clusters (labeled A–D), the same "related items land near each other" structure the toy plot above sketches — Wikimedia Commons, Siobhán Grayson ([link](https://commons.wikimedia.org/wiki/File:T-SNE_visualisation_of_word_embeddings_generated_using_19th_century_literature.png)), CC BY-SA 4.0.*

- **Related words form visible neighbourhoods** (the regions labelled A–D) — the property nearest-neighbour search exploits.
- **t-SNE keeps local neighbours, not global distances.** Read clusters in the plot; do not read the gaps between clusters.

> **Note:** dense and sparse vectors differ in how meaning is stored:
>
> - **Dense:** a few hundred *continuous* numbers, every dimension carrying signal — 16 in our from-scratch encoder, 384 in all-MiniLM.
> - **Sparse (TF-IDF, bag-of-words):** one dimension *per vocabulary word*, almost all zeros, matching only on exact tokens — the embedder that failed the paraphrase test above.
> - **Consequence:** only the dense vector can put two texts with no shared words close together.
...
### One text through the encoder, step by step

The same fixed pipeline turns every text into one stored vector — documents once at index time, the query live at search time:

```mermaid
graph LR
    TXT(["text chunk"]):::data --> TOK(["tokenize<br/>ids: [T]"]):::process
    TOK --> ENC(["transformer encoder<br/>token vectors: [T × d]"]):::process
    ENC --> POOL(["pool tokens → 1<br/>vector: [d]"]):::process
    POOL --> NORM(["L2-normalize<br/>unit vector: [d]"]):::process
    NORM --> VEC(["store (docs)<br/>or search (query)"]):::out
...
What each box does, with all-MiniLM-L6-v2 shapes ($d = 384$):

- **Tokenize:** split into subword ids, capped at the model's maximum length (256 word pieces here). Anything past the cap is dropped — Pitfall 4.
- **Encode:** the transformer returns one contextual vector per token, a $[T \times 384]$ matrix.
- **Pool:** mean-pool (or take `[CLS]`) to collapse $T$ vectors into one $[384]$ vector.
- **Normalize:** divide by the length, so the vector sits on the unit sphere and cosine becomes a plain dot product.
...
With L2-normalized vectors $\mathbf{q}, \mathbf{p} \in \mathbb{R}^{d}$ ($d$ = embedding dimension), cosine similarity collapses to a dot product, $\operatorname{cos}(\mathbf{q}, \mathbf{p}) = \mathbf{q}\cdot\mathbf{p} \in [-1, 1]$. That's the *comparison*. The interesting question is how the encoder is *trained* so this number is high for paraphrases and low otherwise.

### Worked example: cosine by hand, and why normalizing makes every metric agree

"Cosine collapses to a dot product" is a claim worth deriving once, on vectors small enough to check by hand. Take $\mathbf{a} = [1, 0, 1]$ and $\mathbf{b} = [1, 1, 0]$.

**Cosine from its definition** — dot product over the product of lengths:

$$
\cos(\mathbf{a}, \mathbf{b}) = \frac{\mathbf{a}\cdot\mathbf{b}}{\lVert\mathbf{a}\rVert\,\lVert\mathbf{b}\rVert}
= \frac{(1)(1) + (0)(1) + (1)(0)}{\sqrt{1^2+0^2+1^2}\;\sqrt{1^2+1^2+0^2}}
= \frac{1}{\sqrt{2}\,\sqrt{2}} = 0.5
$$

- **Reading it:** the vectors sit $60^\circ$ apart ($\arccos 0.5$) — partly aligned.
- **Source:** the vector-space cosine of [Manning, Raghavan & Schütze (2008), *Introduction to Information Retrieval*, Ch. 6](https://nlp.stanford.edu/IR-book/html/htmledition/scoring-term-weighting-and-the-vector-space-model-1.html).

**Step 1 — normalize, and the dot product *is* the cosine.** Write $\hat{\mathbf{a}} = \mathbf{a}/\lVert\mathbf{a}\rVert$ and $\hat{\mathbf{b}} = \mathbf{b}/\lVert\mathbf{b}\rVert$:

$$
\hat{\mathbf{a}}\cdot\hat{\mathbf{b}} = \frac{\mathbf{a}\cdot\mathbf{b}}{\lVert\mathbf{a}\rVert\,\lVert\mathbf{b}\rVert} = \cos(\mathbf{a}, \mathbf{b})
$$

- **By hand:** $\hat{\mathbf{a}} = [\tfrac{1}{\sqrt2}, 0, \tfrac{1}{\sqrt2}]$, $\hat{\mathbf{b}} = [\tfrac{1}{\sqrt2}, \tfrac{1}{\sqrt2}, 0]$, so $\hat{\mathbf{a}}\cdot\hat{\mathbf{b}} = \tfrac12 = 0.5$ — the same number.
- **Why it matters:** the division by lengths moves out of the similarity and into a one-off normalization step. That is why the encoder above ends with L2-normalize.

**Step 2 — expand the squared distance between unit vectors.** Squared length is a dot product with itself, and $\hat{\mathbf{a}}\cdot\hat{\mathbf{a}} = \hat{\mathbf{b}}\cdot\hat{\mathbf{b}} = 1$:

$$
\lVert\hat{\mathbf{a}} - \hat{\mathbf{b}}\rVert^2
= \hat{\mathbf{a}}\cdot\hat{\mathbf{a}} - 2\,\hat{\mathbf{a}}\cdot\hat{\mathbf{b}} + \hat{\mathbf{b}}\cdot\hat{\mathbf{b}}
= 2 - 2\cos(\mathbf{a}, \mathbf{b})
$$

- **Check by hand:** $\hat{\mathbf{a}} - \hat{\mathbf{b}} = [0, -\tfrac{1}{\sqrt2}, \tfrac{1}{\sqrt2}]$, whose squared length is $\tfrac12 + \tfrac12 = 1$, and $2 - 2(0.5) = 1$.
- **The consequence:** distance falls exactly as cosine rises. Smallest L2 distance, largest cosine and largest dot product give the **same ranking** on unit vectors.

**Step 3 — read the page's own numbers through the identity.** A distance of $\sqrt{2 - 2\cos}$ turns similarities into positions on the sphere:

| Pair (dense encoder, from the worked example below) | cosine | $\lVert\hat{\mathbf{q}} - \hat{\mathbf{p}}\rVert$ |
|---|---|---|
| a paraphrase pair at the mean similarity | 0.675 | 0.806 |
| an unrelated pair at the mean similarity | −0.220 | 1.562 |
| identical direction (the maximum) | 1.000 | 0.000 |
| opposite direction (the minimum) | −1.000 | 2.000 |

Contrastive training is literally moving points across this range: positives toward distance 0, negatives toward distance 2.

The identity verified in PyTorch:
...
cos = (a @ b) / (a.norm() * b.norm())                 # definition
a_hat, b_hat = F.normalize(a, dim=0), F.normalize(b, dim=0)
print(f"{cos.item():.4f} {(a_hat @ b_hat).item():.4f}")                        # cosine vs unit dot
print(f"{(a_hat - b_hat).pow(2).sum().item():.4f} {(2 - 2 * cos).item():.4f}")  # identity, both sides
```
...
> **Note:** the same bound shapes the loss below. Because $\mathbf{q}_i\cdot\mathbf{p}_j \in [-1, 1]$, the InfoNCE logits $\mathbf{q}_i\cdot\mathbf{p}_j/\tau$ live in $[-14.3, 14.3]$ at $\tau = 0.07$ — the temperature, not vector length, sets how sharp the softmax can get.
...
**Dimension is a cost lever, not just a quality knob.** Raw float32 storage is $d \times N \times 4$ bytes — linear in both dimension and corpus size:

| Dimension $d$ (example model) | 1M vectors | 10M vectors | 100M vectors |
|---|---|---|---|
| 256 (a Matryoshka-truncated vector) | 1.0 GB | 10.2 GB | 102 GB |
| 384 (all-MiniLM-L6-v2) | 1.5 GB | 15.4 GB | 154 GB |
| 768 (BERT-base-sized encoders) | 3.1 GB | 30.7 GB | 307 GB |
| 1024 (E5 / BGE large) | 4.1 GB | 41.0 GB | 410 GB |
| 1536 (`text-embedding-3-small`) | 6.1 GB | 61.4 GB | 614 GB |
| 3072 (`text-embedding-3-large`) | 12.3 GB | 123 GB | 1.23 TB |

*Decimal gigabytes ($10^9$ bytes), vectors only, before any index structure is added.*
...
## Pitfalls: the failure modes that silently wreck retrieval
