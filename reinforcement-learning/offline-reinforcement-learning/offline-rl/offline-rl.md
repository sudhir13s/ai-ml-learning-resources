...
1. **Build intuition** — read §1–2 of [Offline RL: Tutorial, Review, and Perspectives](https://arxiv.org/abs/2005.01643). *Why offline RL is hard (distribution shift and out-of-distribution action queries) and the families of fixes, from the authors who framed the problem.*
2. **See the failure named** — read [Off-Policy Deep RL without Exploration (BCQ)](https://arxiv.org/abs/1812.02900). *Extrapolation error: the concrete demonstration that off-policy agents diverge on logged data, and the constrain-to-the-data fix.*
...
## Courses (free)
- [Berkeley CS285 — Offline Reinforcement Learning](http://rail.eecs.berkeley.edu/deeprlcourse/) — **UC Berkeley (Sergey Levine)** — the definitive lectures (Levine's group authored CQL and the survey).
...
- [When Should We Prefer Offline RL Over Behavioral Cloning?](https://arxiv.org/abs/2204.05618) — **Kumar, Hong, Singh & Levine (ICLR 2022)** — the decision that actually comes up in practice: when offline RL beats simply cloning the best trajectories, and when it does not.
...
- Prereq: [06 Q-Learning](/ai-ml/ai-ml-learning-resources/reinforcement-learning/value-based-learning/q-learning/q-learning) · [13 Continuous Control (DDPG/TD3/SAC)](/ai-ml/ai-ml-learning-resources/reinforcement-learning/policy-learning/continuous-control-ddpg-td3-sac/continuous-control-ddpg-td3-sac)
