...
> **Note:** this factorization is **exact**, not an assumption — the chain rule of probability holds for *any* joint distribution. The modeling assumption is only that a transformer with finite parameters can *approximate* each conditional well. (Contrast classic **n-gram** models, which *do* approximate, by truncating the context to the last $n-1$ tokens — see [N-gram Language Models](/ai-ml/ai-ml-learning-resources/multimodal-and-generative-media/natural-language-processing/n-gram-language-models-and-smoothing/n-gram-language-models-and-smoothing). Neural LMs keep the full context.)
...
> **Tip:** to see the real thing at scale, compute perplexity with `model(...).loss` on a held-out text in Hugging Face for a small GPT-2 vs a larger one — the bigger model's perplexity is lower, which is exactly the signal scaling laws are built on (see [Scaling Laws](/ai-ml/ai-ml-learning-resources/models-and-architectures/large-language-models/scaling-laws/scaling-laws)).
...
- **The causal objective is *all* of pre-training for an LLM.** Before any instruction tuning or RLHF, GPT-style models do *nothing but* next-token prediction over trillions of tokens. That single objective is where ~99% of the model's knowledge and capability comes from; everything after ([SFT](/ai-ml/ai-ml-learning-resources/model-adaptation/supervised-fine-tuning/supervised-fine-tuning), [RLHF/DPO](/ai-ml/ai-ml-learning-resources/model-adaptation/preference-and-alignment-training/preference-and-alignment-training)) just *aligns and steers* what next-token prediction already learned.
- **Perplexity is the pre-training dashboard.** During pre-training, the one number everyone watches is held-out perplexity (or its log, the loss) ticking down. The [scaling laws](/ai-ml/ai-ml-learning-resources/models-and-architectures/large-language-models/scaling-laws/scaling-laws) that predict "more compute → better model" are literally fitted curves of *loss vs compute*.
...
**→ [Language Modeling Objectives — references and further reading](/ai-ml/ai-ml-learning-resources/models-and-architectures/large-language-models/language-modeling-objectives/language-modeling-objectives#references-further-reading)**
