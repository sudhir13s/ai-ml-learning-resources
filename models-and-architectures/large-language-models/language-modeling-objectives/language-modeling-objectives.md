...
> **Note:** this factorization is **exact**, not an assumption — the chain rule of probability holds for *any* joint distribution. The modeling assumption is only that a transformer with finite parameters can *approximate* each conditional well. (Contrast classic **n-gram** models, which *do* approximate, by truncating the context to the last $n-1$ tokens — see [N-gram Language Models](../../../../multimodal-and-generative-media/natural-language-processing/n-gram-language-models-and-smoothing/n-gram-language-models-and-smoothing.md). Neural LMs keep the full context.)
