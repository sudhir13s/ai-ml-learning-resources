...
> Companion link library for **[Long-Context Methods](/ai-ml/ai-ml-learning-resources/models-and-architectures/large-language-models/long-context-architectures/long-context-architectures)** (the concept page). This file holds the curated links — external sources *and* internal cross-links — kept separate so it can be reused as a standalone reference list. Grouped by type, best-first. Every entry is free/open and chosen for depth on *this* topic — positional extension (RoPE scaling, YaRN, ALiBi), sparse/sliding attention, and bounded-cache streaming — not popularity.
