...
- explain **bidirectional**, **stacked**, and **seq2seq** RNNs, **teacher forcing / exposure bias**, and the **fixed-context bottleneck** that motivated [attention](/ai-ml/ai-ml-learning-resources/models-and-architectures/attention-and-transformers/attention-mechanism/attention-mechanism);
...
Why not just flatten a sequence and feed it to a feedforward net, or slide a [CNN](/ai-ml/ai-ml-learning-resources/models-and-architectures/classic-architectures/cnns-and-convolution/cnns-and-convolution) over it? Three reasons, and an RNN is built to answer all three at once:
...
The fix was **[attention](/ai-ml/ai-ml-learning-resources/models-and-architectures/attention-and-transformers/attention-mechanism/attention-mechanism)** (Bahdanau et al. 2014): instead of one fixed $c$, let the decoder **look back at *all* the encoder states** at each output step and take a learned weighted average — a soft, content-based lookup over the whole source. That removed the bottleneck, and a few years later [the transformer](/ai-ml/ai-ml-learning-resources/models-and-architectures/attention-and-transformers/transformer-architecture/transformer-architecture) dropped recurrence entirely and kept **only** attention. So this RNN bottleneck is, quite literally, the problem attention was invented to solve — the bridge from this page to the next two. (We don't re-derive attention here; see the [Attention Mechanism](/ai-ml/ai-ml-learning-resources/models-and-architectures/attention-and-transformers/attention-mechanism/attention-mechanism) and [Transformer Architecture](/ai-ml/ai-ml-learning-resources/models-and-architectures/attention-and-transformers/transformer-architecture/transformer-architecture) pages.)
...
- **Streaming / low-latency / on-device.** An RNN keeps an **$O(1)$-per-step** state; a transformer's [KV cache](/ai-ml/ai-ml-learning-resources/inference-and-serving/kv-cache/kv-cache) grows with context. For real-time speech, keyword spotting, and edge inference, that constant-memory recurrence is a real advantage.
...
| **Memory per step at inference** | **$O(1)$** state — cheap streaming | $O(n)$ [KV cache](/ai-ml/ai-ml-learning-resources/inference-and-serving/kv-cache/kv-cache), grows with context |
...
- *What's the modern recurrent revival?* State-space models (S4, **Mamba**) — linear recurrences with a selective state, trainable via parallel scan, $O(1)$-per-step inference, long context. That revival has its own home: [State-Space Models — Foundations](/ai-ml/ai-ml-learning-resources/models-and-architectures/classic-architectures/state-space-models-foundations/state-space-models-foundations).
...
**→ [RNN / LSTM / GRU — references and further reading](/ai-ml/ai-ml-learning-resources/models-and-architectures/classic-architectures/rnn-lstm-gru/rnn-lstm-gru#references-further-reading)**
